...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val userInterrupted: Future[String] = Future.userInput("Input: ").continueWith(f => "You entered... " + Await.result(f, 0 nanos))

    // TO IMPLEMENT
    // 3. create a future that completes after 20 seconds
    //    and continues with a `"Server timeout!"` message
    val timeOut: Future[String] = Future.delay(20 second).continue(_ => "\n Server Timeout!")

    // TO IMPLEMENT
    // 4. create a future that completes when either 10 seconds elapse
    //    or the user enters some text and presses ENTER
    val terminationRequested: Future[String] = Future.delay(10 second).continue(_ => "\n Server Timeout!")
